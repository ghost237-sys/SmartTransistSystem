...
def run_seed(force=False):
    ...
    from scripts.seed_pass_tiers import seed_pass_tiers
    seed_pass_tiers()
    ...
    print('Generating 20 active commuter pass users on Route 111 and Route 105...')
    from domains.passes.models import PassTier, CommuterPass
    
    # Get tiers
    weekly_tier = PassTier.objects.filter(tenant=tenant, tier_type='weekly').first()
    monthly_tier = PassTier.objects.filter(tenant=tenant, tier_type='monthly').first()
    
    for i in range(1, 21):
        username = f'commuter_pass_{i}'
        first_name = f'PassUser{i}'
        last_name = 'Ngong' if i <= 10 else 'Kikuyu'
        
        user = get_or_create_user(username, User.Role.COMMUTER, first_name=first_name, last_name=last_name)
        
        # Assign demo locations on the routes
        if i <= 10:
            assign_demo_location(user, stop_ngong_stage, route_ngong.name)
        else:
            assign_demo_location(user, stop_kikuyu, route_kikuyu.name)
            
        tier = weekly_tier if i % 2 == 0 else monthly_tier
        if tier:
            CommuterPass.objects.filter(user=user, tenant=tenant).delete()
            CommuterPass.objects.create(
                tenant=tenant,
                user=user,
                tier=tier,
                trips_remaining=tier.trip_allowance,
                trips_used=random.randint(0, 5),
                start_date=timezone.now() - timedelta(days=2),
                end_date=timezone.now() + timedelta(days=tier.duration_days - 2),
    ...
    analytics_schedules = [
        ...
        },
        {
            ...
            'fare': Decimal('150'),
            'hours': [5, 6, 7, 8, 12, 17, 18, 19, 20, 21],
            'weekday_trips': (12, 18),
            'weekend_trips': (8, 12),
            'occupancy': (0.75, 0.95),
        },
        ...
        },
        {
            ...
            'fare': Decimal('80'),
            'hours': [6, 7, 8, 9, 12, 15, 16, 17, 18, 19, 20],
            'weekday_trips': (20, 30),
            'weekend_trips': (12, 18),
            'occupancy': (0.70, 0.95),
        },
    ...
    # Ensure at least 3 active trips contain pre-booked (confirmed/boarded) commuters and cash passengers
    print('Generating confirmed/boarded bookings and cash walk-ups on first 3 active trips...')
    for trip in live_trips[:3]:
        # Boarded commuter
        comm = random.choice(commuters)
        b_boarded = Booking.objects.create(
            tenant=tenant,
            trip=trip,
            commuter=comm,
            booking_type=Booking.BookingType.SINGLE,
            ...
            boarded_at=timezone.now() - timedelta(minutes=5),
        )
        b_boarded.generate_ticket_codes()
        b_boarded.save()
        booking_counter += 1
        
        # Confirmed commuter
        comm2 = random.choice([c for c in commuters if c != comm])
        ...
        b_confirmed.generate_ticket_codes()
        b_confirmed.save()
        booking_counter += 1
        
        # Cash passenger (no commuter associated, status boarded)
        b_cash = Booking.objects.create(
            tenant=tenant,
            trip=trip,
            commuter=None,
            booking_type=Booking.BookingType.SINGLE,
    ...
    print('  1. Open the live app link (instantly handshakes as a passwordless guest commuter)')
    ...
    ROUTE_POSITIONS = {
        ...
        'Route 111 (CBD ↔ Ngong)': [(-1.3350, 36.6900, 28)],
        'Route 105 (CBD ↔ Kikuyu)': [(-1.2580, 36.6800, 30)],
    }
    route_counters = {}
